...
def get_most_suitable_x(x_list, coin_list , to_hit, angle_to_hole) :
	...
	for i in range (0,len(x_list)) :
		unob_distance = (to_hit[0] - x_list[i])*(to_hit[0] - x_list[i]) + (to_hit[1] - 145)*(to_hit[1] - 145)
		min_x = min(x_list[i], to_hit[0])
		max_x = max(x_list[i], to_hit[0])
		min_y = min(145, to_hit[1])
		max_y = max(145, to_hit[1])
		
		for j in range(0,len(coin_list)):
			
			if coin_list[j][0] >= min_x and coin_list[j][0] <= max_x and coin_list[j][1] >= min_y and coin_list[j][1] <= max_y :
				distance = dist(coin_list[j],(x_list[i],145))
				
				if distance < unob_distance :
					unob_distance = distance
		
		if unob_distance == dist(to_hit,(x_list[i],145)):
			return x_list[i], unob_distance

		if unob_distance > max_max_travelable_dist : 
			max_max_travelable_dist = unob_distance
			index = i
	
	return x_list[index], max_max_travelable_dist
...
def play(S):
	...
	x_list = range(170,640,10)
	x,dist2 = get_most_suitable_x(x_list,target_list,to_hit, angle_to_hole)
	loc = (x,145)
